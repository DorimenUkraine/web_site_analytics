...
menu = [{'url': '.index', 'title': 'Панель'},
        ...
        {'url': '.listintegrations', 'title': 'Список интеграций'},
...
def listintegrations():
    ...
    if db:
        try:
            cur = db.cursor()
            cur.execute(f"SELECT ga_id FROM settings")
            list = cur.fetchall()
        except sqlite3.Error as e:
            print("Ошибка получения статей из БД " + str(e))

    return render_template('admin/listintegrations.html', title='Список интеграций', menu=menu, list=list)
